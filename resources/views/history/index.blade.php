<x-layout title="Reading History - MangaMongo">
    <div class="min-h-screen bg-gradient-to-b from-gray-950 to-black py-6 sm:py-8 px-3 sm:px-4">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="mb-6 sm:mb-8">
                <div class="flex flex-col gap-4">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <div>
                            <h1 class="text-2xl sm:text-3xl lg:text-4xl font-bold text-text mb-1 sm:mb-2">Reading History</h1>
                            <p class="text-sm sm:text-base text-gray-400">Total {{ $histories->total() }} manga</p>
                        </div>

                        <div class="flex flex-wrap gap-2 sm:gap-3">
                            <a href="{{ route('profile.show') }}" 
                               class="inline-flex items-center justify-center gap-2 bg-gray-800 hover:bg-gray-700 text-text font-semibold px-4 sm:px-6 py-2 sm:py-2.5 rounded-lg transition-all hover:scale-105 text-sm sm:text-base flex-1 sm:flex-initial">
                            ...
                            @if($histories->count() > 0)
                                <button onclick="if(confirm('Hapus semua history?')) document.getElementById('clear-history-form').submit()" 
                                        class="inline-flex items-center justify-center gap-2 bg-red-500/90 hover:bg-red-600 text-text font-semibold px-4 sm:px-6 py-2 sm:py-2.5 rounded-lg transition-all hover:scale-105 text-sm sm:text-base flex-1 sm:flex-initial">
                                    <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @if($histories->count() > 0)
                <div class="space-y-3 sm:space-y-4">
                    @foreach($histories as $history)
                        <div class="bg-gray-900/50 border border-gray-800 rounded-lg sm:rounded-xl p-3 sm:p-4 hover:border-accent/50 hover:bg-gray-900/70 transition-all duration-300 group">
                            <div class="flex gap-3 sm:gap-4">
                                ...
                                <a href="{{ route('manga.read', [$history->manga->slug, $history->chapter_number]) }}" 
                                   class="shrink-0 relative overflow-hidden rounded-lg group/thumb">
                                    ...
                                    <div class="absolute inset-0 bg-black/0 group-hover/thumb:bg-black/30 transition-colors flex items-center justify-center">
                                        <svg class="w-6 h-6 sm:w-8 sm:h-8 text-text opacity-0 group-hover/thumb:opacity-100 transition-opacity" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div class="flex-1 min-w-0 flex flex-col justify-between">
                                    <div>
                                        <a href="{{ route('manga.detail', $history->manga->slug) }}" 
                                           class="block group/title">
                                            <h3 class="text-text font-bold text-sm sm:text-base lg:text-lg mb-1.5 sm:mb-2 group-hover/title:text-accent-hover transition-colors line-clamp-2 sm:line-clamp-1">
                                                {{ $history->manga->title }}
                                            </h3>
                                        </a>

                                        {{-- Genres - Hide on very small screens --}}
                                        <div class="hidden sm:flex flex-wrap gap-1.5 sm:gap-2 mb-2 sm:mb-3">
                                            @foreach($history->manga->genres->take(3) as $genre)
                                                <span class="px-2 py-0.5 sm:py-1 bg-linear-to-r from-accent/20 to-accent-hover/20 text-amber-400 rounded text-xs font-medium">
                                                    {{ $genre->name }}
                                                </span>
                                            @endforeach
                                        </div>
                                    ...
                                    <div class="flex flex-col gap-1.5 sm:gap-2">
                                        <a href="{{ route('manga.read', [$history->manga->slug, $history->chapter_number]) }}" 
                                           class="inline-flex items-center gap-1.5 sm:gap-2 text-text hover:text-accent-hover transition-colors text-xs sm:text-sm">
                                ...
                                <div class="hidden sm:flex flex-col gap-2 ml-2">
                                    <a href="{{ route('manga.read', [$history->manga->slug, $history->chapter_number]) }}" 
                                       class="px-4 lg:px-5 py-2 bg-accent hover:bg-accent-hover text-text font-bold rounded-lg transition-all hover:scale-105 text-center text-sm whitespace-nowrap shadow-lg shadow-amber-500/20">
                                        Lanjut Baca
                                    </a>
                                    ...
                                    <form method="POST" action="{{ route('history.destroy', $history->id) }}" class="inline">
                                        ...
                                        <button type="submit" 
                                                ...
                                                class="w-full px-4 lg:px-5 py-2 bg-red-500/90 hover:bg-red-600 text-text font-bold rounded-lg transition-all hover:scale-105 text-sm whitespace-nowrap">
                                            Hapus
                                        </button>
                                    </form>
                                </div>
                            ...
                            <div class="sm:hidden flex gap-2 mt-3 pt-3 border-t border-gray-800">
                                ...
                                <form method="POST" action="{{ route('history.destroy', $history->id) }}" class="flex-shrink-0">
                                    ...
                                    <button type="submit" 
                                            onclick="return confirm('Hapus dari history?')"
                                            class="px-4 py-2 bg-red-500/90 hover:bg-red-600 text-text font-bold rounded-lg transition-all text-sm">
            ...
            @else
                ...
                <div class="bg-gray-900/50 border border-gray-800 rounded-xl sm:rounded-2xl p-8 sm:p-12 text-center">
                    ...
                    <h2 class="text-xl sm:text-2xl font-bold text-text mb-2 sm:mb-3">Belum Ada History</h2>
