    ...
    <div class="min-h-screen bg-linear-to-b from-card-2 to-transparent">
        ...
        <div class="px-4 sm:px-6 lg:px-8 py-4">
            <nav class="flex items-center gap-2 text-sm text-text-second">
                ...
                <span>/</span>
                <span class="text-text">{{ $manga->title }}</span>
            </nav>
        </div>
        ...
        <div class="px-4 sm:px-6 lg:px-8 pb-10">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div class="sticky top-24">
                        ...
                        <div class="relative rounded-xl overflow-hidden shadow-2xl mb-4 md:mx-auto md:max-w-xs">
                            @if($manga->rating >= 8.5)
                                <div class="absolute top-3 left-3 z-10">
                                    <span class="bg-linear-to-r from-red-600 to-orange-500 text-text text-xs font-bold px-3 py-1.5 rounded-full">
                        ...
                        <div class="grid grid-cols-2 gap-3 mb-4">
                            <button type="button"
                                    ...
                                    class="bookmark-toggle bg-card hover:bg-card-3 border border-border text-text rounded-xl px-4 py-3 flex items-center justify-center gap-2 transition-colors">
                            ...
                            @auth
                               <button onclick="openRatingModal()" 
                                        class="bg-card hover:bg-card-3 border border-border text-text rounded-xl px-4 py-3 flex items-center justify-center gap-2 transition-colors {{ $manga->isRatedBy(Auth::user()) ? 'border-accent' : '' }}">
                                    <svg class="w-5 h-5 {{ $manga->isRatedBy(Auth::user()) ? 'text-rating fill-current' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            @else
                                <button onclick="showRatingLoginModal()" 
                                        class="bg-card hover:bg-card-3 border border-border text-text rounded-xl px-4 py-3 flex items-center justify-center gap-2 transition-colors">
                        ...
                        <button
                            type="button"
                            id="share-button"
                            class="share-button w-full bg-card hover:bg-card-3 border border-border text-text rounded-xl px-4 py-3 flex items-center justify-center gap-2 transition-colors mb-4 focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2 focus:ring-offset-gray-950 disabled:opacity-50 disabled:cursor-not-allowed"
                        ...
                        @if($manga->chapters->count() > 0)
                            <a href="{{ route('manga.read', [$manga->slug, $manga->chapters->first()->number]) }}" 
                               class="w-full bg-accent hover:bg-accent-hover text-text font-bold rounded-xl px-4 py-3 text-center transition-colors flex items-center justify-center gap-2">
                                 <svg class="w-6 h-6 text-gray-800 dark:text-text" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.03v13m0-13c-2.819-.831-4.715-1.076-8.029-1.023A.99.99 0 0 0 3 6v11c0 .563.466 1.014 1.03 1.007 3.122-.043 5.018.212 7.97 1.023m0-13c2.819-.831 4.715-1.076 8.029-1.023A.99.99 0 0 1 21 6v11c0 .563-.466 1.014-1.03 1.007-3.122-.043-5.018.212-7.97 1.023"/>
                                </svg>
                                <span class="text-sm font-semibold">Baca Dari Awal</span>
                            </a>
                        @else
                            <div class="w-full bg-card-3 text-gray-500 font-bold rounded-xl px-4 py-3 text-center cursor-not-allowed flex items-center justify-center gap-2">
                                <svg class="w-6 h-6 text-gray-800 dark:text-text" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.03v13m0-13c-2.819-.831-4.715-1.076-8.029-1.023A.99.99 0 0 0 3 6v11c0 .563.466 1.014 1.03 1.007 3.122-.043 5.018.212 7.97 1.023m0-13c2.819-.831 4.715-1.076 8.029-1.023A.99.99 0 0 1 21 6v11c0 .563-.466 1.014-1.03 1.007-3.122-.043-5.018.212-7.97 1.023"/>
                                </svg>
                ...
                <div class="lg:col-span-9">
                    {{-- Title & Rating --}}
                    <div class="mb-6">
                        <h1 class="text-3xl md:text-4xl font-bold text-text mb-3">{{ $manga->title }}</h1>
                        ...
                        {{-- Meta Info --}}
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 bg-card-detail border border-border rounded-xl p-4">
                            <div>
                                <p class="text-text-second text-sm mb-1">Author:</p>
                                <p class="text-text font-medium">{{ $manga->author ?? 'Unknown' }}</p>
                            </div>
                            <div>
                                <p class="text-text-second text-sm mb-1">Artist:</p>
                                <p class="text-text font-medium">{{ $manga->artist ?? 'Unknown' }}</p>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <p class="text-text font-medium">{{ $manga->type ?? 'Manga' }}</p>
                            </div>
                        </div>

                        {{-- Stats --}}
                        <div class="grid grid-cols-3 gap-4 mt-4">
                            <div class="bg-card/50 border border-border rounded-xl p-4 text-center shadow-black/25">
                                <p class="text-2xl font-bold text-text">{{ $manga->formatted_views }}</p>
                                <p class="text-text-second text-sm mt-1">Views</p>
                            </div>
                            <div class="bg-card/50 border border-border rounded-xl p-4 text-center">
                                <p class="text-2xl font-bold text-text">{{ $manga->bookmarks->count() }}</p>
                                <p class="text-text-second text-sm mt-1">Bookmark</p>
                            </div>
                            <div class="bg-card/50 border border-border rounded-xl p-4 text-center cursor-pointer hover:border-accent transition-colors"
                                onclick="showRatings()">
                                <div class="flex items-center justify-center gap-1">
                                    <p class="text-2xl font-bold text-text">{{ $manga->average_rating ?? 0 }}</p>
                                    <svg class="w-6 h-6 text-rating" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div class="mb-8">
                        <div class="border-b border-border">
                            <nav class="flex gap-6 overflow-auto">
                                <button onclick="showTab('chapters')" id="tab-chapters" class="tab-button text-text border-b-2 border-accent-hover px-1 py-3 font-bold">
                                    Chapters
                                </button>
                                <button onclick="showTab('comments')" id="tab-comments" class="tab-button text-text-second hover:text-text border-b-2 border-transparent px-1 py-3 transition-colors">
                                    Comments
                                </button>
                                <button onclick="showTab('details')" id="tab-details" class="tab-button text-text-second hover:text-text border-b-2 border-transparent px-1 py-3 transition-colors">
                                    Details
                                </button>
                                <button onclick="showTab('gallery')" id="tab-gallery" class="tab-button text-text-second hover:text-text border-b-2 border-transparent px-1 py-3 transition-colors">
                                    Gallery
                                </button>
                            </nav>
                        </div>
                    ...
                    <div id="content-chapters" class="tab-content">
                        {{-- Sinopsis --}}
                        <div class="bg-card-detail border border-border rounded-xl p-6 mb-6">
                            <h2 class="text-xl font-bold text-text mb-3">Sinopsis</h2>
                        ...
                        {{-- Chapter List --}}
                        <div id="chapters" class="bg-card-detail border border-border rounded-xl p-6">
                            <div class="flex items-center justify-between mb-4">
                                <h2 class="text-xl font-bold text-text">Daftar Chapter</h2>
                                <select id="chapterSort" class="bg-card-3 border border-border text-text rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent/70">
                            ...
                            <div class="mb-4">
                                <input 
                                    ...
                                    autocomplete="off"    
                                    class="w-full bg-card-3 border border-border rounded-lg px-4 py-2 text-text placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-accent/70"
                                >
                            </div>
                                ...
                                @foreach($manga->chapters->sortByDesc('number') as $chapter)
                                    <div class="chapter-item" 
                                        ...
                                        <a href="{{ route('manga.read', [$manga->slug, $chapter->number]) }}" 
                                           class="block bg-card-3/50 hover:bg-card-3 border border-border hover:border-accent rounded-lg p-4 transition-all group">
                                            <div class="flex items-center justify-between">
                                                <div class="flex-1">
                                                    <h3 class="text-text font-medium group-hover:text-accent-hover transition-colors">
                    ...
                    <div id="content-comments" class="tab-content hidden">
                        <div class="bg-card-detail border border-border rounded-xl p-6">
                           <div class="sm:flex items-center justify-between mb-2 sm:mb-6">
                            ...
                            @auth
                                <form method="POST" action="{{ route('comments.store.manga', $manga) }}" class="mb-8">
                                    ...
                                    <textarea name="content" 
                                            ...
                                            class="w-full px-4 py-3 bg-card-3 border border-border rounded-lg text-text placeholder-gray-500 focus:outline-none focus:border-accent resize-none"></textarea>
                                    ...
                                    <div class="flex items-center justify-between mt-3">
                                        ...
                                        <button type="submit"
                                            class="px-4 py-2 text-sm sm:px-6 sm:py-2.5 sm:text-base
                                                bg-accent hover:bg-accent-hover text-text font-bold rounded-2xl sm:rounded-lg transition-colors">
                            ...
                            @else
                                <div class="bg-card-detail border border-border rounded-lg p-6 text-center mb-8">
                    ...
                    <div id="content-details" class="tab-content hidden">
                        <div class="bg-card-detail border border-border rounded-xl p-6">
                            <h2 class="text-2xl font-bold text-text mb-6">Detail Manga</h2>
                                ...
                                <div>
                                    ...
                                    <div class="space-y-3">
                                        <div>
                                            ...
                                            <p class="text-text">{{ $manga->alternative_title ?? $manga->title }}</p>
                                        </div>
                                        <div>
                                            <p class="text-text-second text-sm mb-1">Type:</p>
                                            <p class="text-text">{{ $manga->type ?? 'Manga' }}</p>
                                        </div>
                                        <div>
                                            <p class="text-text-second text-sm mb-1">Pengarang:</p>
                                            <p class="text-text">{{ $manga->author ?? 'Unknown' }}</p>
                                        </div>
                                        <div>
                                            <p class="text-text-second text-sm mb-1">Artis:</p>
                                            <p class="text-text">{{ $manga->artist ?? 'Unknown' }}</p>
                                        </div>
                                        <div>
                                            <p class="text-text-second text-sm mb-1">Status:</p>
                                            <p class="text-text">{{ strtoupper($manga->status) }}</p>
                                        </div>
                                        <div>
                                            <p class="text-text-second text-sm mb-1">Tahun Terbit:</p>
                                            <p class="text-text">{{ $manga->released_at ?? 'Unknown' }}</p>
                                        </div>
                                        <div>
                                            <p class="text-text-second text-sm mb-1">Di Serialisasi Oleh:</p>
                                            <p class="text-text">{{ $manga->serialization ?? 'Unknown' }}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div class="space-y-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-text">{{ number_format($manga->views) }} kali</p>
                                        </div>
                                        <div>
                                            <p class="text-text-second text-sm mb-1">Bookmark:</p>
                                            <p class="text-text">{{ $manga->bookmarks->count() }}</p>
                                        </div>
                                        <div>
                                            <p class="text-text-second text-sm mb-1">Jumlah Chapter:</p>
                                            <p class="text-text">{{ $manga->chapters->count() }}</p>
                                        </div>
                                        <div>
                            ...
                            @if($manga->genres->count() > 0)
                                <div class="mt-6">
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        @foreach($manga->genres as $genre)
                                            <span class="bg-card-3 hover:bg-accent text-gray-300 hover:text-text text-sm px-3 py-1.5 rounded-lg border border-border hover:border-accent transition-colors cursor-pointer">
                                                {{ $genre->name }}
                                            </span>
                                        @endforeach
                                    </div>
                    ...
                    <div id="content-gallery" class="tab-content hidden">
                        <div class="bg-card-detail border border-border rounded-xl p-6">
                            <div class="flex items-center justify-between mb-6">
                                <h2 class="text-2xl font-bold text-text">Gallery</h2>
                            ...
                            @if($manga->galleries->count() > 0)
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                    @foreach($manga->galleries as $index => $gallery)
                                        <div class="group relative aspect-3/4 rounded-lg overflow-hidden bg-card-3 cursor-pointer"
                                            ...
                                            <div class="absolute top-2 left-2">
                                                <span class="bg-accent text-text text-xs font-bold px-2 py-1 rounded">
                                                ...
                                                @if(Auth::user()->isAdmin())
                                                    <div class="absolute top-2 right-2 flex gap-1">
                                                        <button onclick="event.stopPropagation(); editGallery({{ $gallery->id }})"
                                                                class="p-1.5 bg-blue-500 hover:bg-blue-600 text-text rounded transition-colors">
                                                        ...
                                                        <form method="POST" action="{{ route('admin.gallery.destroy', $gallery) }}" class="inline"
                                                            ...
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit" class="p-1.5 bg-red-500 hover:bg-red-600 text-text rounded transition-colors">
                                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div id="lightbox" class="hidden fixed inset-0 bg-black/80
                                 z-50 flex items-center justify-center p-4">
                                    <button onclick="closeLightbox()" 
                                            class="absolute top-4 right-4 text-text hover:text-accent-hover transition-colors z-10">
                                    ...
                                    <button onclick="previousImage()" 
                                            class="absolute left-4 text-text hover:text-accent-hover transition-colors">
                                    ...
                                    <div class="max-w-6xl max-h-full flex flex-col items-center ">
                                        ...
                                        <div id="lightbox-info" class="mt-4 text-center text-text">
                                    ...
                                    <button onclick="nextImage()" 
                                            class="absolute right-4 text-text hover:text-accent-hover transition-colors">
                                        <svg class="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                                        </svg>
                                    </button>
    ...
    <script>
        function showTab(tabName) {
            ...
            document.querySelectorAll('.tab-button').forEach(button => {
                button.classList.remove('text-accent-hover', 'border-accent-hover', 'font-bold');
                button.classList.add('text-text', 'border-transparent', 'font-normal', 'hover:text-text');
            });
            ...
            activeTab.classList.remove('text-text-second', 'border-transparent', 'font-normal', 'hover:text-text');
            activeTab.classList.add('text-accent-hover', 'border-accent-hover', 'font-bold');
        }
