<x-layout title="Lupa Password" title="Login" :noNav="true" :noFooter="true" :noPadding="true">
    <div class="min-h-screen flex items-center justify-center bg-gray-900 px-4">
        <div class="bg-gray-800 p-8 rounded-2xl shadow-xl w-full max-w-md">
            ...
            <p class="text-text-second mb-6 text-center">
            ...
            <form method="POST" action="{{ route('password.email') }}">
                ...
                <div class="mb-6">
                    ...
                    <input type="email" 
                           ...
                           required 
                           autofocus
                           class="w-full px-4 py-3 rounded-lg bg-gray-700 text-white border border-gray-600 focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/50">
                    @error('email')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                
                <button type="submit" class="w-full bg-accent hover:bg-accent-hover text-black font-bold py-3 px-4 rounded-lg transition-colors">
                    Kirim Link Reset
                </button>
                
                <p class="text-text-second text-center mt-6">
                    <a href="{{ route('login') }}" class="text-accent-hover hover:text-amber-300">
                        Kembali ke Login
                    </a>
                </p>
            </form>
