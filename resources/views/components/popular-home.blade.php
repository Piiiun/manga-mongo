...
<section class="relative mt-12 px-4 sm:px-8">
    ...
    <div class="mb-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <a href="{{ route('manga.list', ['sort' => 'popular']) }}" 
               class="hidden sm:flex group items-center gap-2 rounded-full bg-gradient-to-r from-amber-500/10 to-red-500/10 px-5 py-2.5 font-semibold text-amber-400 ring-1 ring-amber-500/30 transition-all hover:ring-amber-500/60">
    ...
    <div class="space-y-3 sm:space-y-4">
        @foreach ($popularMangas as $index => $manga)
            <article class="group relative flex items-center gap-3 sm:gap-4 overflow-hidden rounded-xl bg-gradient-to-r from-gray-800/80 to-gray-900/80 backdrop-blur-sm transition-all duration-300 hover:from-gray-800 hover:to-gray-900 hover:shadow-xl hover:shadow-amber-500/10">
                ...
                <div class="relative z-10 flex h-full items-center">
                    <div class="flex h-20 sm:h-24 w-12 sm:w-16 items-center justify-center">
                        <span class="text-2xl sm:text-3xl font-black {{ $index === 0 ? 'text-amber-500' : ($index === 1 ? 'text-gray-400' : ($index === 2 ? 'text-orange-600' : 'text-gray-500')) }}">
                            {{ $index + 1 }}
                        </span>
                    </div>
                </div>
                ...
                <div class="relative z-10 flex flex-1 flex-col gap-1.5 sm:gap-2 py-3 sm:py-4 pr-3 sm:pr-4">
                    ...
                    <div class="flex items-start gap-2">
                        <a href="{{ route('manga.detail', $manga->slug) }}" class="group/title flex-1">
                            <h3 class="line-clamp-2 text-sm sm:text-base font-bold text-white transition-colors group-hover/title:text-amber-400">
                                {{ $manga->title }}
                            </h3>
                        </a>
                        
                        @if ($manga->rating >= 8.0)
                            <span class="shrink-0 rounded bg-gradient-to-r from-red-600 to-red-500 px-1.5 sm:px-2 py-0.5 sm:py-1 text-xs font-bold text-white shadow-lg">
                                HOT
                            </span>
                        @endif
                    </div>
                    ...
                    <div class="hidden xs:flex flex-wrap gap-1.5 sm:gap-2">
                        @foreach ($manga->genres->take(2) as $genre)
                            <span class="rounded bg-amber-500/20 px-2 sm:px-2.5 py-0.5 text-xs font-semibold text-amber-400 ring-1 ring-amber-500/30">
                                {{ $genre->name }}
                            </span>
                        @endforeach
                    </div>
                    ...
                    {{-- Stats - Mobile Layout --}}
                    <div class="flex items-center gap-3 sm:hidden text-xs text-gray-400">
                        {{-- Rating --}}
                        <div class="flex items-center gap-1">
                            ...
                            <span class="font-bold text-amber-400">{{ number_format($manga->rating ?? 8.5, 1) }}</span>
                        </div>
